...
export class LogiqueStrategy extends EclatMicroserviceStrategy implements CustomTransportStrategy {
    ...
    private configureServer(): void {
        ...
        this.server.all('/', async (req, res) => {
            const messageName: string = req.headers['x-message-type'] as string;

            const handler: MessageHandler | null = this.getHandlerByPattern(messageName);
            if (!handler) {
                throw new BadRequestException();
            }

            const { messageType } = this.getHandlerSpecByName(messageName);

            return this.runInContext(
                plainToClass(messageType, req.body),
                handler,
                { request: req }
            );
        });
